<script>
  ...
  async function handleTaskFormSubmit() {
    ...
    const endpoint = currentTask.id
      ? `/api/tasks/${currentTask.id}`
      : `/api/workspaces/${currentWorkspaceId}/tasks`;
...
{:else if tasks.length === 0}
  <div class="text-center py-12 bg-white rounded-lg shadow-sm">
    ...
    <button
      ...
      class="mt-4 bg-pink-600 text-white font-semibold py-2 px-4 rounded-lg"
    >
...
{:else}
  {#each tasks as task (task.id)}
    <div class="bg-white p-4 rounded-lg shadow-sm flex items-start gap-4">
      <input
        type="checkbox"
        class="mt-1 h-5 w-5 rounded border-gray-300 text-pink-600 focus:ring-pink-500"
      />
      <div class="flex-1">
      ...
      <div class="flex items-center gap-2">
        <button
          on:click={() => handleOpenTaskModal(task)}
  ...
  >
    ...
    >
      <h3 class="text-lg font-bold mb-4">
        {currentTask.id ? "Sửa công việc" : "Thêm công việc mới"}
      </h3>
      <form on:submit|preventDefault={handleTaskFormSubmit} class="space-y-4">
        <div>
          <label for="task-title" class="block text-sm font-medium"
            >Tiêu đề</label
          >
          <input
            type="text"
        ...
        <div>
          <label for="task-desc" class="block text-sm font-medium"
