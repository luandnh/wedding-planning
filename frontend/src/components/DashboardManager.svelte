<script>
  ...
  // Nhận props cần thiết từ Astro
  export let user;
  export let token;

  // Quản lý state ở client
  let isLoading = true;
  let error = null;
  let data = null;
  ...
  async function loadDashboardData() {
    ...
    try {
      ...
      const dashboardRes = await fetch(
        `${backendUrl}/api/workspaces/${currentWorkspaceId}/dashboard`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (!dashboardRes.ok) throw new Error("Không thể tải dữ liệu tổng quan.");

      data = await dashboardRes.json();

      // Xử lý ngày cưới sau khi có dữ liệu
      const mainEventKeywords = ["cưới", "vu quy", "tân hôn", "thành hôn"];
      if (data.config && data.config.importantDates) {
        const mainDate = data.config.importantDates.find((d) =>
          mainEventKeywords.some((k) => d.name.toLowerCase().includes(k))
        );
  ...
  $: budgetPercent =
    data && data.budget.totalExpected > 0
      ? (data.budget.totalActual / data.budget.totalExpected) * 100
      : 0;
...
{:else if data}
  <div class="space-y-6">
    ...
    <!-- (MỚI) Tên Cô dâu & Chú rể -->
    <div
      class="bg-white p-6 rounded-xl shadow-sm text-center"
      in:fly={{ y: -20, duration: 500, delay: 50, easing: quintOut }}
    >
      <div class="flex items-center justify-center gap-4">
        <h2 class="text-3xl md:text-4xl title-font text-slate-700">
          {data.config.groomName || "Chú Rể"}
        </h2>
        <span class="text-2xl text-pink-400 title-font">&</span>
        <h2 class="text-3xl md:text-4xl title-font text-slate-700">
          {data.config.brideName || "Cô Dâu"}
        </h2>
      </div>
    </div>
    ...
    {#if weddingDate && !isNaN(weddingDate)}
      <div
        class="bg-gradient-to-r from-pink-500 to-rose-500 text-white p-6 rounded-xl shadow-lg text-center"
        in:fly={{ y: -20, duration: 500, delay: 100, easing: quintOut }}
      >
        <h3 class="font-semibold text-lg opacity-90">
          Đếm ngược đến ngày cưới
        </h3>
        <div class="grid grid-cols-4 gap-2 mt-4">
          <div>
            <p class="text-3xl font-bold">{countdown.days}</p>
            <p class="text-xs opacity-80">Ngày</p>
          </div>
          <div>
            <p class="text-3xl font-bold">{countdown.hours}</p>
            <p class="text-xs opacity-80">Giờ</p>
          </div>
          <div>
            <p class="text-3xl font-bold">{countdown.minutes}</p>
            <p class="text-xs opacity-80">Phút</p>
          </div>
          <div>
            <p class="text-3xl font-bold">{countdown.seconds}</p>
            <p class="text-xs opacity-80">Giây</p>
          </div>
        </div>
      </div>
    {/if}
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div class="md:col-span-2 space-y-6">
            ...
            <div>
              <div class="flex justify-between text-sm mb-1">
                ...
                <span class="font-bold text-pink-600"
                  >{formatCurrency(data.budget.totalActual)}</span
                >
              </div>
              <div class="w-full bg-slate-200 rounded-full h-2.5">
                <div
                  class="bg-pink-600 h-2.5 rounded-full transition-all duration-1000"
                  style="width: {budgetPercent}%"
                ></div>
              </div>
              <p class="text-right text-xs text-slate-500 mt-1">
                Dự kiến: {formatCurrency(data.budget.totalExpected)}
              </p>
            </div>
          </div>
        </div>
        ...
        >
          <div class="flex justify-between items-center mb-3">
            ...
            <a
              href="/tasks"
              class="text-sm text-pink-600 hover:underline font-medium"
              >Xem tất cả</a
            >
          </div>
          {#if data.tasks.upcoming.length > 0}
            <ul class="space-y-3">
              {#each data.tasks.upcoming as task}
                ...
                >
                  <span
                    class="h-2.5 w-2.5 rounded-full {task.priority === 'high'
                      ? 'bg-red-400'
      ...
      <div class="space-y-6">
        <div
          class="bg-white p-4 rounded-lg shadow-sm text-center"
        ...
        <div
          class="bg-white p-4 rounded-lg shadow-sm text-center"
          in:fly={{ y: -20, duration: 500, delay: 350, easing: quintOut }}
        >
