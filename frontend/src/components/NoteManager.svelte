<script>
  ...
  async function handleNoteFormSubmit() {
    ...
    await apiCall(
      `/api/workspaces/${currentWorkspaceId}/notes`,
      "POST",
      payload
    );
    showNoteModal = false;
...
{#if showNoteModal}
  ...
  >
    <form
      on:submit|preventDefault={handleNoteFormSubmit}
    ...
    >
      ...
      <div>
        ...
        <input
          ...
          accept="image/*"
          class="mt-1 text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-pink-50 file:text-pink-700 hover:file:bg-pink-100"
        />
