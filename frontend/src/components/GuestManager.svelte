<script>
  ...
  function handleOpenGuestModal(guest = null) {
    currentGuest = guest
      ? { ...guest }
      : { id: null, name: "", phone: "", side: "groom", status: "pending" };
    showGuestModal = true;
  }

  async function handleGuestFormSubmit() {
    if (!currentGuest.name || !currentGuest.side) return;
    const endpoint = currentGuest.id
      ? `/api/guests/${currentGuest.id}`
      : `/api/workspaces/${currentWorkspaceId}/guests`;
            ...
            <li class="py-2 flex justify-between items-center">
              <div>
                ...
                <p class="text-sm text-slate-500">
                  {guest.phone || "Chưa có SĐT"}
                </p>
              </div>
              <div class="flex items-center gap-2">
                <button
                  on:click={() => handleOpenGuestModal(guest)}
                ...
                >
                <button
                  on:click={() => handleDeleteGuest(guest.id)}
...
<div class="fixed bottom-24 right-4 z-10">
  <button
    on:click={() => handleOpenGuestModal()}
    class="bg-pink-600 text-white w-14 h-14 rounded-full shadow-lg flex items-center justify-center text-3xl font-light"
    >+</button
  >
</div>
...
{#if showGuestModal}
  <div
    ...
    on:click={() => (showGuestModal = false)}
  >
    <form
      on:submit|preventDefault={handleGuestFormSubmit}
      class="bg-white p-6 rounded-xl shadow-lg w-full max-w-md space-y-4"
      on:click|stopPropagation
    >
      <h3 class="text-lg font-bold">
        {currentGuest.id ? "Sửa thông tin khách mời" : "Thêm khách mời mới"}
      </h3>
      <div>
        <label for="guest-name" class="block text-sm font-medium"
          >Họ và tên</label
        >
        <input
          type="text"
          id="guest-name"
          required
          bind:value={currentGuest.name}
          class="mt-1 w-full px-3 py-2 border border-slate-300 rounded-lg"
        />
      </div>
      <div>
        <label for="guest-phone" class="block text-sm font-medium"
          >Số điện thoại</label
        >
        <input
          type="tel"
          id="guest-phone"
          bind:value={currentGuest.phone}
          class="mt-1 w-full px-3 py-2 border border-slate-300 rounded-lg"
        />
      </div>
      <div>
        ...
        <div class="mt-2 flex gap-4">
          <label class="flex items-center"
            ><input
              type="radio"
              bind:group={currentGuest.side}
              value="groom"
              class="text-pink-600"
            /> <span class="ml-2">Nhà trai</span></label
          >
          <label class="flex items-center"
            ><input
              type="radio"
              ...
              value="bride"
              class="text-pink-600"
            /> <span class="ml-2">Nhà gái</span></label
          >
        </div>
      </div>
      <div class="mt-6 flex justify-end gap-3">
        <button
          type="button"
          on:click={() => (showGuestModal = false)}
          class="px-4 py-2 bg-slate-200 rounded-lg">Hủy</button
        >
        <button
          type="submit"
          class="px-4 py-2 bg-pink-600 text-white rounded-lg"
          >Lưu khách mời</button
        >
      </div>
    </form>
  </div>
